...
function BotFace({ mood, blinking }: { mood: BotMood; blinking: boolean }) {
  if (blinking || mood === 'sleeping') return (
    <svg viewBox="0 0 36 36" width="36" height="36" style={{ color: 'var(--color-text)' }}>
      ...
      {mood === 'sleeping' && <text x="26" y="10" fontSize="8" fill="var(--color-muted)">z</text>}
    </svg>
  )
...
/** Get ALL elements matching selector, excluding fixed/sticky ancestors */
function getPageEls(selector: string): HTMLElement[] {
...
export function DevBot() {
  ...
  const eatRandomWord = useCallback(() => {
    const els = getPageEls('p, h3, h2, h1, li')
  ...
  const flipHeading = useCallback(() => {
    ...
    const fix = () => {
      target.style.transform = 'scaleY(1)'
      setTimeout(() => { target.style.transform = ot; target.style.transition = otr; target.style.display = od }, 400)
  ...
  const wobbleEl = useCallback(() => {
    const els = getPageEls('h1,h2,h3,button,p,li').filter(el => el.getBoundingClientRect().width > 40)
    if (!els.length) return
    const target = pick(els)
    const ot = target.style.transform, otr = target.style.transition, od = target.style.display
    target.style.display = 'inline-block'
    ...
    setTimeout(() => { target.style.transform = ot; target.style.transition = otr; target.style.display = od }, 9 * 80 + 200)
    speak('wiggle wiggle! 🕺', 2000)
    setMoodTemp('happy', 2000)
  }, [speak, setMoodTemp])

  // 5. Glow any section/div
  const glowEl = useCallback(() => {
    ...
    setMoodTemp('excited', 2000)
    setTimeout(() => { target.style.outline = oo; target.style.boxShadow = os; target.style.transition = ot }, 2500)
  }, [speak, setMoodTemp])

  // 6. Troll button
  const trollButton = useCallback(() => {
    const els = getPageEls('button').filter(el => {
    ...
    setTimeout(() => { if (target.isConnected) { target.innerHTML = origHTML; target.style.transition = ot; target.style.transform = otr } }, 3000)
  }, [speak, setMoodTemp])
  ...
  const shakeEl = useCallback(() => {
    ...
    xs.forEach((v, i) => { setTimeout(() => { target.style.transform = `translate(${v}px,${ys[i]}px)` }, i * 60) })
    setTimeout(() => { target.style.transition = ot; target.style.transform = otr }, xs.length * 60 + 100)
  ...
  const scrambleText = useCallback(() => {
    const els = getPageEls('h2,h3,p,span,li,button')
      .filter(el => {
        const t = el.textContent?.trim() ?? ''
    ...
    const iv = setInterval(() => {
      if (frame >= totalFrames) { clearInterval(iv); if (target.isConnected) target.textContent = originalText; return }
      if (!target.isConnected) { clearInterval(iv); return }
        ...
        {bubble && (
          <motion.div
            ...
            style={{
              ...
              padding: '6px 12px', fontSize: 12, color: 'var(--color-text)',
